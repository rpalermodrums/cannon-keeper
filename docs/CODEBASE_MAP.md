...
**RPC namespaces**: `project.*`, `system.*`, `search.*`, `scenes.*`, `issues.*`, `style.*`, `bible.*`, `canon.*`, `export.*`, `jobs.*`
**Patterns**:
...
**Key features**:
- Per-action busy tracking (`Map<ActionNamespace, Set<ActionLabel>>`) with `isBusy(namespace)` per view
- Error queue (`errors[]` max 5, per-error dismiss via `dismissError(id)`)
- Boot state machine (`"booting"` → `"ready"` | `"restore-failed"`)
- Per-domain loaded flags (`scenesLoaded`, `issuesLoaded`, `styleLoaded`, `entitiesLoaded`)
- Auto-refresh on worker idle (busy→idle transition via `refreshProjectData`)
...
| View | Purpose |
|------|---------|
| `DashboardView` | Document progress, evidence backing, continue actions, notices |
...
| `IssuesView` | Issue triage with sort/filter, timestamps, scene cross-links, inline resolve confirm |
| `StyleView` | Repetition table, tone drift, dialogue tics with scene cross-links and show-more toggle |
| `BibleView` | Characters & World with evidence-backed claims |
| `ExportView` | Markdown/JSON export |
| `SettingsView` | Runtime health, diagnostics, workspace memory, processing queue |
...
- Per-action busy tracking replaced flat `busy: string[]`; use `isBusy(namespace)` to check per-view loading state
